...
=======
    ...
    usage: isfread.py [-h] [-v] [-d] [-o OUTPUT_FILE] INPUT_FILE
...
Initial author: Gustavo Pasquevich, Universidad Nacional de La Plata - Argentina
...
__version__ = "0.6.1"
__author__ = "Gustavo Pasquevich, Ralf Ahlbrink"
...
def isfread(filename, verbose=False, debug=False):
    ...
    with open(filename, "rb") as fileInput:
        ...
        bytesHeader = bytesHeader[
            0 : (intLocationOfColon + keylength + 1 + intNofDigits)
        ]
        ...
        if older_version:
            tupleHeader = {
                "bytenum": getnum(bytesHeader, "BYT_NR"),
            ...
            tupleHeader = {
                "bytenum": getnum(bytesHeader, "BYT_N"),
        ...
        if debug:
            print("DEBUG information ----------")
            print(
                "    intNofDataBytes = %d, intCalculatedNofBytes = %d"
                % (intNofDataBytes, intCalculatedNofBytes)
            )
        ...
        _string_data2 = fileInput.read(intCalculatedNofBytes)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="show verbose message (default: False)",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    parser.add_argument(
        "-o",
        "--out",
        help="output data as OUTPUT_FILE.csv",
        type=str,
        metavar="OUTPUT_FILE",
    )
    args = parser.parse_args()

    ndarrayValue, tupleHeader = isfread(
        args.INPUT_FILE, verbose=args.verbose, debug=args.debug
    )
